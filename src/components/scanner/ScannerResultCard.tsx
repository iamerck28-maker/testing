...
import ShareSignal from '@/components/scanner/ShareSignal';
...
export default function ScannerResultCard({
  ...
  return (
    <div
      className="relative flex w-full flex-col gap-2.5 rounded-[14px] border border-border-line bg-surface-card p-3 text-left transition-colors hover:border-text-muted/30 active:bg-surface-card/80"
    >
      {/* Share button — top right, above the clickable card area */}
      <div className="absolute right-2 top-2 z-10">
        <ShareSignal pick={pick} scannerName={scannerName} />
      </div>
      ...
      <button
        ...
        aria-label={`View ${pick.symbol} details`}
      >
